...
def process(elig, cap, valid_br):
    ...
    for _, row in all_c.iterrows():
        ...
        # Rule 7: target branch
        # Customer with branch in lookup -> stay at their branch
        # Customer with branch NOT in lookup -> skip (not our population)
        # Customer with NO branch -> use true branch if valid
        if cb:
            if cb in valid_br:
                target_br = cb
            else:
                continue  # skip — branch not in our lookup, not our population
        ...
        # No valid branch available (branchless customer with no valid true branch)
        if not target_br:
            rec.update(Rec_Ofc="REVIEW", Move="Needs Review", Reason="No valid branch available",
                       Flag="No current branch and true branch empty or not in branch lookup")
            rec["Rec_Ofc_Display"] = "REVIEW"
            recs.append(rec); continue

        # Flag if branchless customer using true branch
        if not cb:
