...
def process(elig, cap, valid_br):
    ...
    while go:
        go = False
        for oc, info in oinfo.items():
            amask = rdf["Rec_Ofc"]==oc
            if amask.sum()==0: continue
            ac = rdf.loc[amask].sort_values("SB")
            low = ac.iloc[0]
            umask = (rdf["Rec_Ofc"]=="Unassigned")&(rdf["Rec_Br"]==info["br"])
            if umask.sum()==0: continue
            uc = rdf.loc[umask].sort_values("SB",ascending=False)
            high = uc.iloc[0]
            if high["SB"]>low["SB"]:
...
def write_output(rdf, cust, cap):
    ...
    rc = ["CK","CN","CC","RK","RR","Tier","TRV","SB","Dep","Loan","Eq","Ins",
    ...
    rh = ["Cust Key","Cust Name","Cust Class","Relationship Key","Relationship Role",
          "Balance Tier","Total Relationship Value","Sortable Balance","Deposits","Loans","Equity","Insurance",
    ...
    rc7 = ["CK","CN","CC","Tier","SB","TRV","Dep","Eq","Ins","Cur_Br","Cur_Ofc",
           "Rec_Ofc","Rec_Name","Rec_RC","Rec_Br","Reason","Flag"]
    rh7 = ["Cust Key","Cust Name","Cust Class","Balance Tier","Sortable Balance",
    ...
    for br in sorted(cap):
        for rt in sorted(cap[br]):
            ...
            n = len(ocs); tc = lim*n
            cnt = int(rdf["Rec_Ofc"].isin(ocs).sum())
            val = float(rdf.loc[rdf["Rec_Ofc"].isin(ocs),"SB"].sum())
            brs.append({"Br":br,"Role":rt,"Num":n,"CapPer":lim,"TotCap":tc,
                        "Cnt":cnt,"Open":tc-cnt,"Val":round(val,2)})
    bdf = pd.DataFrame(brs)
    write_sheet(wb.create_sheet("Branch Role Summary"),
                ["Branch","Role","Num Officers","Cap Per Officer","Total Capacity","Customer Count","Open Slots","Portfolio Value"],
                df_rows(bdf, ["Br","Role","Num","CapPer","TotCap","Cnt","Open","Val"]))
    ...
    for br in sorted(cap):
        for rt in sorted(cap[br]):
            ...
            for o in cap[br][rt]:
                m = rdf["Rec_Ofc"]==o["oc"]
                cnt, val = int(m.sum()), float(rdf.loc[m,"SB"].sum())
                rps.append({"Br":br,"OC":o["oc"],"ON":o["on"],"Role":rt,"RC":o["rc"],
                            "Cap":lim,"Cnt":cnt,"Open":lim-cnt,"Val":round(val,2)})
    rpdf = pd.DataFrame(rps)
    write_sheet(wb.create_sheet("RM Portfolio Summary"),
                ["Branch","Officer Code","Officer Name","Role","Role Code","Capacity","Customer Count","Open Slots","Portfolio Value"],
                df_rows(rpdf, ["Br","OC","ON","Role","RC","Cap","Cnt","Open","Val"]))
